...
| 08 | [`08-20260427-zg-storage-agents`](./08-20260427-zg-storage-agents) | 0G | ⚠️ **Currently broken end-to-end** — see folder README. Phase 1 of the agentcompanies.io spec: an agent is `AGENTS.md` + `SOUL.md` + a `manifest.json` whose hash IS the agent's permanent identity. UI saves the bundle to 0G Storage, loads any agent by hash, and chats with it through 0G Compute. Code matches the official 0G SDK docs but the testnet upload is intermittently failing (TCP filter from the dev laptop's ISP; contract revert from a clean GCP route) — multiple builders reporting in the hackathon Discord/Telegram. Committed as-is so the design contribution and downstream chat path are reviewable; the path becomes end-to-end the moment Storage is healthy. |
| 09 | [`09-20260428-0g-tool-calling-test`](./09-20260428-0g-tool-calling-test) | 0G | ✓ Single-shot test that confirms 0G Compute passes OpenAI-compatible tool-calling end-to-end. Sends a chat completion with a `get_weather` function tool to qwen2.5-7b-instruct via the broker; response comes back with `finish_reason: tool_calls` and a properly-shaped `tool_calls` array. Unblocks the autonomous-companies `SKILL.md` design — citizens can run real agent loops with native tools, and MCP integration becomes a clean shim instead of a research project. |
