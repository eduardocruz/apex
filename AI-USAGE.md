...
| 2026-04-28 | `phase2/onboarding/twin/server.js` (remove /api/rebind) | generated | Claude Code (Opus 4.7) | Removed the rebind endpoint and the registry "Claim ownership" button after Eduardo flagged the security hole: anyone could claim any unverified citizen. New rule documented in code comment: owner is set only at mint time via signature; legacy/un-signed citizens become orphaned and are evicted by heartbeat tax (Constitution Article III §2). |
